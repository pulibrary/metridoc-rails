...
ActiveAdmin.register Tools::FileUploadImport do
  ...
  controller do
    def create
      ...
      flash[:notice] = t("flash.actions.create.notice", resource_name: @file_import.model_name.human) if @file_import.save
    end
  end
  ...
  show do |file_upload_import|
    attributes_table do
      ...
      row :uploaded_file do
        link_to file_upload_import.uploaded_file.filename, url_for(file_upload_import.uploaded_file), target: '_blank', rel: 'noopener' if file_upload_import.status == 'success'
      end
  ...
  index do
    ...
    column :uploaded_file do |file_upload_import|
      link_to file_upload_import.uploaded_file.filename, url_for(file_upload_import.uploaded_file), target: '_blank', rel: 'noopener'
    end
